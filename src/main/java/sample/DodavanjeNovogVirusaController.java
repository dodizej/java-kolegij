...
import main.niti.DodavanjeNovogVirusaNit;
...
public class DodavanjeNovogVirusaController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        List<String> naziviSimptoma = Glavna.getBazaPodataka().getAllSimptomi()
                .stream().map(s -> s.getNaziv()).collect(Collectors.toList());
    ...
    public void spremi() {
        ...
        if (!nazivField.getText().isBlank() && !odabraniSimptomi.isEmpty()) {
            ...
            service.execute(new DodavanjeNovogVirusaNit(tempNaziv, odabraniSimptomi));
